...
public class AuthController {
    ...
    public ResponseEntity<?> createResetPasswordToken(@RequestParam("email") String email) {
        try {
            com.thullo.web.payload.response.TokenResponse passwordResetToken = authService.createPasswordResetTokenForUser(email);
    ...
    public ResponseEntity<?> resendVerificationToken(@RequestParam("token") String token) {
        try {
            TokenResponse vToken = authService.resendVerificationToken(token);
            ResponseEntity<ApiResponse> methodLinkBuilder = methodOn(AuthController.class)
                    .verifyUser(vToken.getToken());
    ...
    public ResponseEntity<?> resendResetPasswordToken(@RequestParam("token") String token) {
        try {
            TokenResponse vToken = authService.resendResetPasswordToken(token);
