...
public class UserServiceImpl implements UserService {
     ...
     */
    @Override
    public UserResponse getUserDetails(String email) throws UserException {
     ...
     */
    @Override
    public List<UserResponse> searchUserProfiles(String searchQuery) {
        List<User> users = userRepository.findByParams(searchQuery);
        List<UserResponse> profiles = new ArrayList<>();
        users.forEach(user -> {
            UserResponse profile = mapper.map(user, UserResponse.class);
